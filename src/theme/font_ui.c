...
static uint32_t font_ui_find(uint32_t codepoint) {
    const struct inkcell_font_ui_table *table = &inkcell_font_ui_table;
    uint32_t low = 0U;
...
static bool font_ui_has_glyph(uint32_t codepoint) {
    return font_ui_find(codepoint) < inkcell_font_ui_table.count;
}
...
static uint8_t font_ui_advance(uint32_t codepoint) {
    ...
    return index < inkcell_font_ui_table.count ? inkcell_font_ui_table.glyphs[index].advance : 0U;
}
...
static void font_ui_tofu(uint8_t *out) {
    const int master_w = (int)inkcell_font_ui_table.master_w;
    const int inset = (int)inkcell_font_ui_table.nominal / 8;
    ...
    const int right = (int)inkcell_font_ui_table.nominal - inset - 1;
...
const struct inkcell_font *inkcell_font_ui(void) {
    ...
    font.cap_rows = inkcell_font_ui_table.cap_rows;
    font.master_w = inkcell_font_ui_table.master_w;
    font.master_left = inkcell_font_ui_table.master_left;
    font.proportional = inkcell_font_ui_table.proportional;
    font.name = inkcell_font_ui_table.name;
    ...
    font.nominal = inkcell_font_ui_table.nominal;
    font.width = (uint8_t)((inkcell_font_ui_table.nominal + INKCELL_FONT_UI_MASTER_SCALE / 2) /
