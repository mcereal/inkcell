...
static void font_ui_tofu(uint8_t *out) {
    const int inset = INKCELL_FONT_UI_MASTER_W / 8;
    const int left = inset;
    const int right = INKCELL_FONT_UI_MASTER_W - inset - 1;
    ...
    for (int y = top; y <= bottom; ++y) {
        for (int x = left; x <= right; ++x) {
            ...
            if (edge) {
                out[(size_t)y * INKCELL_FONT_UI_MASTER_W + (size_t)x] = INKCELL_GLYPH_MAX_ALPHA;
            }
        }
    }
}
...
static bool font_ui_glyph(uint32_t codepoint, struct inkcell_glyph *out) {
    ...
    for (uint32_t row = 0U; row < glyph->h; ++row) {
        uint8_t *dst = &out->alpha[((size_t)glyph->y + row) * INKCELL_FONT_UI_MASTER_W + glyph->x];
...
const struct inkcell_font *inkcell_font_ui(void) {
    ...
    static struct inkcell_font font = {
        .id = "ui",
        .name = "JetBrains Mono",
        .width = INKCELL_FONT_UI_CELL_W,
        .height = INKCELL_FONT_UI_CELL_H,
        .advance_gap = 0U,
        .line_gap = 1U,
        .master_w = INKCELL_FONT_UI_MASTER_W,
