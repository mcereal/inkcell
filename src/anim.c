...
static int32_t ease_out_cubic(int32_t t) {
    /* 1 - (1 - t)^3 */
    const int64_t inv = INKCELL_ANIM_ONE - t;
    const int64_t cubed = inv * inv * inv;
    return (int32_t)(INKCELL_ANIM_ONE -
                     cubed / ((int64_t)INKCELL_ANIM_ONE * (int64_t)INKCELL_ANIM_ONE));
}

static int32_t ease_in_out_cubic(int32_t t) {
    if (t < INKCELL_ANIM_ONE / 2) {
        /* 4t^3 */
        const int64_t cubed = (int64_t)t * t * t;
        return (int32_t)(4 * cubed / ((int64_t)INKCELL_ANIM_ONE * (int64_t)INKCELL_ANIM_ONE));
    }
    /* 1 - (-2t + 2)^3 / 2 */
    const int64_t inv = 2 * (int64_t)(INKCELL_ANIM_ONE - t);
    const int64_t cubed = inv * inv * inv;
    return (int32_t)(INKCELL_ANIM_ONE -
                     cubed / (2 * (int64_t)INKCELL_ANIM_ONE * (int64_t)INKCELL_ANIM_ONE));
}
...
static int32_t clamp_progress(int32_t value) {
    ...
    if (value > INKCELL_ANIM_ONE) {
        return INKCELL_ANIM_ONE;
    }
    return value;
}

int32_t inkcell_ease(enum inkcell_ease ease, int32_t progress) {
    ...
    if (t == 0 || t == INKCELL_ANIM_ONE) {
        return t;
    }
    switch (ease) {
    case INKCELL_EASE_OUT:
        return clamp_progress(ease_out_cubic(t));
    case INKCELL_EASE_IN_OUT:
        return clamp_progress(ease_in_out_cubic(t));
    case INKCELL_EASE_LINEAR:
    case INKCELL_EASE_COUNT:
    default:
        return t;
    }
}

int32_t inkcell_anim_value(const struct inkcell_anim *anim, uint64_t now_ms) {
    if (anim == NULL) {
        return 0;
    }
    ...
    const int32_t linear = (int32_t)((elapsed * (uint64_t)INKCELL_ANIM_ONE) / anim->duration_ms);
    const int32_t eased = inkcell_ease((enum inkcell_ease)anim->ease, linear);
    const int32_t span = anim->to - anim->from;
    return clamp_progress(anim->from + (int32_t)(((int64_t)span * eased) / INKCELL_ANIM_ONE));
}

bool inkcell_anim_active(const struct inkcell_anim *anim, uint64_t now_ms) {
...
void inkcell_anim_set(struct inkcell_anim *anim, int32_t to) {
    if (anim == NULL) {
        return;
    }
...
void inkcell_anim_to(struct inkcell_anim *anim, uint64_t now_ms, int32_t to, uint32_t duration_ms,
                     enum inkcell_ease ease) {
    if (anim == NULL) {
        return;
    }
    ...
    anim->from = inkcell_anim_value(anim, now_ms);
    anim->to = target;
    anim->start_ms = now_ms;
    anim->duration_ms = duration_ms;
    anim->ease = (uint8_t)(ease < INKCELL_EASE_COUNT ? ease : INKCELL_EASE_LINEAR);
}

/* ---- the keyed table --------------------------------------------------------------------- */

static struct inkcell_anim_slot *table_slot(struct inkcell_anim_table *table, uint32_t id,
                                            uint64_t now_ms, bool *created) {
    struct inkcell_anim_slot *free_slot = NULL;
    struct inkcell_anim_slot *oldest = &table->slots[0];

    for (size_t i = 0; i < INKCELL_ANIM_SLOTS; ++i) {
        struct inkcell_anim_slot *slot = &table->slots[i];
        if (slot->id == id) {
    ...
    struct inkcell_anim_slot *slot = free_slot != NULL ? free_slot : oldest;
...
int32_t inkcell_anim_track(struct inkcell_anim_table *table, uint32_t id, uint64_t now_ms,
                           int32_t to, uint32_t duration_ms, enum inkcell_ease ease) {
    if (table == NULL || id == 0U) {
        return clamp_progress(to);
    }

    bool created = false;
    struct inkcell_anim_slot *slot = table_slot(table, id, now_ms, &created);
    ...
    if (created || slot->loop) {
        slot->loop = false;
        inkcell_anim_set(&slot->anim, to);
    } else {
        inkcell_anim_to(&slot->anim, now_ms, to, duration_ms, ease);
    }
    return inkcell_anim_value(&slot->anim, now_ms);
}

int32_t inkcell_anim_loop(struct inkcell_anim_table *table, uint32_t id, uint64_t now_ms,
                          uint32_t period_ms) {
    ...
    return (int32_t)((elapsed % (uint64_t)period_ms) * (uint64_t)INKCELL_ANIM_ONE /
                     (uint64_t)period_ms);
}

bool inkcell_anim_table_active(const struct inkcell_anim_table *table, uint64_t now_ms) {
    if (table == NULL) {
        return false;
    }
    for (size_t i = 0; i < INKCELL_ANIM_SLOTS; ++i) {
        const struct inkcell_anim_slot *slot = &table->slots[i];
        if (slot->id == 0U) {
            continue;
        }
        /* A loop has no end to reach, so what keeps it running is that something is still
           ...
           INKCELL_ANIM_LOOP_STALE_MS. */
        if (slot->loop) {
            if (now_ms <= slot->touched_ms + (uint64_t)INKCELL_ANIM_LOOP_STALE_MS) {
                return true;
            }
            continue;
        }
        if (inkcell_anim_active(&slot->anim, now_ms)) {
            return true;
        }
    }
    return false;
...
void inkcell_anim_table_reset(struct inkcell_anim_table *table) {
    if (table != NULL) {
