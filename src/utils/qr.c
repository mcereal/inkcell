...
/*
 * QR Code encoding, byte mode. See inkcell/utils/qr.h for what this is for and what it is not.
 *
...
static size_t data_codewords(int version, enum inkcell_qr_ecc ecc) {
...
static void set_module(struct inkcell_qr *qr, int x, int y, bool dark, bool function) {
...
static bool is_dark(const struct inkcell_qr *qr, int x, int y) {
    if (x < 0 || y < 0 || x >= (int)qr->size || y >= (int)qr->size) {
        return false;
    }
...
static bool is_function(const struct inkcell_qr *qr, int x, int y) {
    return (qr->modules[(size_t)y * qr->size + (size_t)x] & QR_MODULE_FUNCTION) != 0U;
}

static void draw_finder(struct inkcell_qr *qr, int cx, int cy) {
...
static void draw_alignment(struct inkcell_qr *qr, int cx, int cy) {
...
static void draw_format(struct inkcell_qr *qr, enum inkcell_qr_ecc ecc, int mask) {
...
static void draw_version(struct inkcell_qr *qr, int version) {
    if (version < 7) {
        return;
    }
...
static void draw_function_patterns(struct inkcell_qr *qr, int version, enum inkcell_qr_ecc ecc) {
    const int size = (int)qr->size;
...
static void draw_codewords(struct inkcell_qr *qr, const uint8_t *data, size_t len) {
    const int size = (int)qr->size;
    size_t bit = 0U;
...
static void apply_mask(struct inkcell_qr *qr, int mask) {
    const int size = (int)qr->size;
 ...
 */
static long penalty_runs(const struct inkcell_qr *qr, bool horizontal) {
    const int size = (int)qr->size;
    long penalty = 0;
...
static long penalty_score(const struct inkcell_qr *qr) {
    const int size = (int)qr->size;
...
static size_t interleave(const uint8_t *data, int version, enum inkcell_qr_ecc ecc, uint8_t *out) {
...
bool inkcell_qr_dark(const struct inkcell_qr *qr, int x, int y) {
