...
const char *inkcell_log_level_to_string(enum inkcell_log_level level) {
    switch (level) {
    case INKCELL_LOG_LEVEL_TRACE:
        return "TRACE";
    case INKCELL_LOG_LEVEL_DEBUG:
        return "DEBUG";
    case INKCELL_LOG_LEVEL_INFO:
        return "INFO";
    case INKCELL_LOG_LEVEL_WARN:
        return "WARN";
    case INKCELL_LOG_LEVEL_ERROR:
        return "ERROR";
    case INKCELL_LOG_LEVEL_NONE:
        return "NONE";
    }
    return "UNKNOWN";
}
...
#define INKCELL_LOG_SINK_LINE_MAX 256U
...
void inkcell_log_message_v(enum inkcell_log_level level, const char *component, const char *fmt,
    ...
    fprintf(stderr, "%s [%s]", timestamp, inkcell_log_level_to_string(level));
    if (component != NULL && component[0] != '\0') {
...
/* One step of the scan and of the shift. Small on purpose: together they cover at most
   INKCELL_LOG_FILE_KEEP_BYTES once each, at startup, and a bigger buffer would buy nothing a card's
...
bool inkcell_log_file_default_path(char *out, size_t out_len) {
    ...
    const char *override = inkcell_env_get("LOG_FILE");
...
long inkcell_log_file_compact(const char *path) {
    ...
    if (!S_ISREG(info.st_mode) || info.st_size <= (off_t)INKCELL_LOG_FILE_MAX_BYTES) {
        (void)close(fd);
        return 0L;
    }
    ...
    const off_t resume =
        log_file_line_start(fd, info.st_size - (off_t)INKCELL_LOG_FILE_KEEP_BYTES, info.st_size);
    /*
