 ...
 */
#define INKCELL_FB_METER_MOTION INKCELL_MOTION_LONG
...
#define INKCELL_FB_METER_LOOP_MOTION INKCELL_MOTION_LOOP
...
#define INKCELL_FB_METER_PILL_PCT 34
 ...
 * validated against INKCELL_COLOR_METER_TRACK is a bar that vanishes on some theme somebody has
 ...
 */
static enum inkcell_tone inkcell_fb_meter_tone(enum inkcell_tone tone) {
    return inkcell_tone_family(tone) != INKCELL_FAMILY_COUNT ? tone : INKCELL_TONE_PRIMARY;
}
 ...
 */
static int inkcell_fb_band_mark(const struct inkcell_fb_meter *meter, int32_t boundary) {
    const int32_t permille = inkcell_scale_permille(meter->scale, boundary);
    if (permille <= 0 || permille >= INKCELL_ANIM_ONE) {
        return -1;
    }
...
void inkcell_fb_draw_meter(struct inkcell_backend_fb_state *state,
    ...
    if (meter->kind == INKCELL_FB_METER_INDETERMINATE) {
        ...
        if (w > 0) {
            inkcell_fb_fill_round_rect(state, x, r.y, w, r.h, radius, ink);
        }
        return;
    }
     ...
     */
    const int32_t value = inkcell_scale_permille(meter->scale, meter->value);
    ...
    const struct inkcell_rgb ink = inkcell_fb_tone_color(
        state, inkcell_fb_meter_tone(inkcell_band_tone(meter->band, meter->value, meter->tone)));

    /* Where the fill has got to, which is not where the reading is: see INKCELL_FB_METER_MOTION. */
    const int32_t position =
        inkcell_anim_track(&state->anim, meter->id, state->now_ms, value,
                           inkcell_fb_motion(state, INKCELL_FB_METER_MOTION), INKCELL_EASE_OUT);

    int fill = (int)(((int64_t)r.w * position) / INKCELL_ANIM_ONE);
    if (fill <= 0) {
    ...
    if (fill > 0) {
        inkcell_fb_fill_round_rect(state, r.x, r.y, fill, r.h, radius, ink);
    }
    ...
    for (size_t i = 0U; i < sizeof bounds / sizeof bounds[0]; i++) {
        const int mark = inkcell_fb_band_mark(meter, bounds[i]);
        if (mark < 0) {
            continue;
        }
        ...
        if (w > 0) {
            inkcell_fb_fill_rect(state, x, r.y, w, r.h, ground);
        }
    }
}
 ...
 */
#define INKCELL_FB_SLIDER_MOTION INKCELL_MOTION_SHORT
 ...
 * short inside it.
 */
#define INKCELL_FB_SLIDER_TRACK 2
...
static void inkcell_fb_slider_stops(const struct inkcell_backend_fb_state *state,
    ...
    for (uint32_t stop = 1U; stop + 1U < stops; ++stop) {
        ...
        if (x >= track->x && x + notch <= track->x + track->w) {
            inkcell_fb_fill_rect(state, x, track->y, notch, track->h, ground);
        }
    }
}
...
void inkcell_fb_draw_slider(struct inkcell_backend_fb_state *state,
    ...
    const int full = box.h * 2 / INKCELL_FB_SLIDER_HANDLE_FOCUS;
    const int thickness = full > 1 ? full : 1;
    const struct inkcell_fb_rect track = {
    ...
    if (target < 0) {
        target = 0;
    } else if (target > INKCELL_ANIM_ONE) {
        target = INKCELL_ANIM_ONE;
    }
    const int32_t position =
    ...
    if (active > 0) {
        inkcell_fb_fill_round_rect(state, track.x, track.y, active, track.h, radius, ink);
    }
...
void inkcell_fb_draw_signal(const struct inkcell_backend_fb_state *state,
     ...
     */
    const int steps = (int)INKCELL_SIGNAL_STEPS;
...
void inkcell_fb_draw_sparkline(const struct inkcell_backend_fb_state *state,
    ...
    const struct inkcell_polyline *points = spark->points;
    ...
    const struct inkcell_fb_rect r = spark->rect;
    const int stroke = inkcell_fb_spark_stroke(state->scale);
    ...
    for (uint32_t i = 0U; i < points->count && i < INKCELL_SERIES_MAX; ++i) {
        const struct inkcell_point *point = &points->items[i];
        const int x = r.x + (int)(((int64_t)point->x * span) / INKCELL_ANIM_ONE);
 ...
 * other and each states its own count - the same split a bounded name field makes across the
...
void inkcell_fb_draw_proportion(const struct inkcell_backend_fb_state *state,
    ...
    const struct inkcell_fb_rect r = bar->rect;

    int32_t widths[INKCELL_PROPORTION_PARTS];
    const uint32_t parts = inkcell_proportion_split(bar->values, bar->count, r.w, widths);
    if (parts == 0U) {
     ...
     *
     * In the caller's ground rather than in INKCELL_COLOR_BG, which is where this differs from
...
#define INKCELL_FB_CHART_FOOTER_LINES 2
 ...
 * INKCELL_SERIES_COLORS may never be used as an ink. A stroke half the glyph scale wide is not a
...
int inkcell_fb_chart_min_height(const struct inkcell_backend_fb_state *state,
    ...
    /* The chrome, and a plot at least as tall again as the chrome under it. Below that the
       ...
       small chart. */
    return layout->line * (INKCELL_FB_CHART_FOOTER_LINES * 2);
}
...
static void inkcell_fb_chart_legend(const struct inkcell_backend_fb_state *state,
    ...
    for (uint32_t i = 0U; i < chart->count && i < INKCELL_FB_CHART_LINES; ++i) {
        ...
        if (label == INKCELL_STR_NONE && value == NULL) {
            continue;
        }
...
uint32_t inkcell_fb_chart_reading_rows(const struct inkcell_backend_fb_state *state,
    ...
    if (rect->h < inkcell_fb_chart_min_height(state, layout)) {
        return 0U;
    }
    int width = 0;
    struct inkcell_fb_rect body = *rect;
    body.h -= inkcell_fb_chart_strip(state, layout, rect, spans, &width);
...
void inkcell_fb_draw_chart(const struct inkcell_backend_fb_state *state,
    ...
    if (chart->rect.h < inkcell_fb_chart_min_height(state, layout)) {
        return; /* see inkcell_fb_chart_min_height(): there is no clipped chart */
    }

    const int scale = layout->small;
    const int adv = inkcell_fb_char_adv(state, scale);
    const int rule = inkcell_fb_chart_rule(state->scale);
    ...
    if (chart->readings != NULL && chart->reading_count > 0U) {
        inkcell_fb_draw_chart_readings(state, layout, chart, &body);
        return;
    }
