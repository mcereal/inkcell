...
inkcell_fb_card_measure(const struct inkcell_backend_fb_state *state,
    ...
    m.heading_h = card->heading[0] != '\0' ? inkcell_fb_line_adv(state, layout->small) : 0;
    /*
     ...
     */
    m.button_h = card->action_count > 0U ? inkcell_fb_line_adv(state, layout->small) : 0;
 ...
 * wants a body.
 */
static uint32_t inkcell_fb_card_row_lines(const struct inkcell_fb_card_row *row, size_t cols) {
    if (row->kind != INKCELL_FB_CARD_ROW_NOTE) {
        return 1U;
    }
    uint32_t lines = inkcell_wrap_lines(row->value, cols);
    if (lines > INKCELL_FB_CARD_NOTE_LINES) {
        lines = INKCELL_FB_CARD_NOTE_LINES;
    }
...
static struct inkcell_fb_card_fit inkcell_fb_card_clip(const struct inkcell_fb_card_metrics *m,
    ...
    if (fit.rows < card->count && card->rows[fit.rows].kind == INKCELL_FB_CARD_ROW_NOTE) {
        const uint32_t whole = inkcell_fb_card_row_lines(&card->rows[fit.rows], m->cols);
        for (uint32_t lines = whole > 0U ? whole - 1U : 0U; lines > 0U; --lines) {
            const struct inkcell_fb_card_fit probe = {.rows = fit.rows, .tail_lines = lines};
            if (top + inkcell_fb_card_box_height(m, layout, card, probe) <= bottom) {
...
static struct inkcell_fb_card_row *inkcell_fb_card_next_row(struct inkcell_fb_card *card,
    ...
    struct inkcell_fb_card_row *row = &card->rows[card->count++];
...
void inkcell_fb_card_row(struct inkcell_fb_card *card, enum inkcell_tone tone,
    ...
    va_list args;
    va_start(args, value);
    (void)inkcell_str_vformat(text, sizeof text, value, args);
    va_end(args);
...
int inkcell_fb_card_height(const struct inkcell_backend_fb_state *state,
    ...
    const struct inkcell_fb_card_fit whole = {.rows = card->count, .tail_lines = 0U};
    return inkcell_fb_card_box_height(&m, layout, card, whole);
}
...
static uint32_t inkcell_fb_draw_card_row(struct inkcell_backend_fb_state *state,
    ...
    if (row->kind == INKCELL_FB_CARD_ROW_METER) {
        /*
         ...
         * about what the bar is for; see inkcell_fb_card_meter().
         *
         ...
         */
        const int adv = inkcell_fb_char_adv(state, state->scale);
        const int bar_x = inkcell_fb_card_row_label(state, m, y, row, ground);
        const int bar_right = m->content_x + (int)m->cols * adv;
        const int height = inkcell_fb_meter_thickness(state, state->scale);
    ...
    if (row->kind == INKCELL_FB_CARD_ROW_PROPORTION) {
        ...
        const int bar_x = inkcell_fb_card_row_label(state, m, y, row, ground);
        const int bar_right = m->content_x + (int)m->cols * adv;
        const int height = inkcell_fb_proportion_thickness(state, state->scale);
        if (bar_right - bar_x > 0) {
            ...
            for (uint32_t i = 0; i < row->part_count && i < INKCELL_PROPORTION_PARTS; ++i) {
                bar.values[i] = row->parts[i];
            }
            inkcell_fb_draw_proportion(state, &bar);
        }
        return 1U;
    }
...
bool inkcell_fb_draw_card_reserving(struct inkcell_backend_fb_state *state,
     ...
     * to reach it.
     */
    const int bottom = layout->footer_y - inkcell_fb_margin(state) / 2;
    /*
    ...
    struct inkcell_fb_card_fit fit = inkcell_fb_card_clip(&m, layout, card, *y, limit);
    if (fit.rows == 0U && fit.tail_lines == 0U) {
         ...
         */
        fit = inkcell_fb_card_clip(&m, layout, card, *y, bottom);
    }
    if (fit.rows == 0U && fit.tail_lines == 0U) {
        return false;
    }
