/*
 * The frame a chart draws inside: see include/inkcell/ui/trend.h for why it is one file rather than
...
static const struct {
    ...
    [INKCELL_TREND_SPAN_6H] = {6U * 60U * 60U * 1000U, INKCELL_STR_TREND_SPAN_6H},
    [INKCELL_TREND_SPAN_ALL] = {0U, INKCELL_STR_TREND_SPAN_ALL},
};
...
struct inkcell_scale inkcell_trend_domain(struct inkcell_scale domain, int32_t high) {
    ...
    const int32_t max = domain.min == domain.max ? INKCELL_ANIM_ONE : domain.max;
    if (max <= min) {
    ...
    for (size_t i = 0U; i < sizeof k_rungs / sizeof k_rungs[0]; ++i) {
        ...
        return (struct inkcell_scale){min, (int32_t)ceiling};
    }
    return domain;
}
 ...
 * `gap_ms`, or after a inkcell_series_break() - is emitted by the projection and drawn by
 ...
 */
static bool series_high(const struct inkcell_series *const *series, uint32_t count, uint32_t from,
    ...
    for (uint32_t i = 0U; i < count; ++i) {
        ...
        for (uint32_t j = 0U; j < one->count; ++j) {
            const struct inkcell_sample *sample = inkcell_series_at(one, j);
             ...
             * about it - that is inkcell_series_project_within()'s own rule, and asking the raw
             * break test here instead would count a reading as drawn because of a predecessor the
             * clip has already thrown away.
             */
            const bool starts = here == 0U || inkcell_series_starts_segment(one, j);
            bool drawn = !starts;
            if (!drawn) {
                const struct inkcell_sample *next = inkcell_series_at(one, j + 1U);
                drawn =
                    next != NULL && next->time <= to && !inkcell_series_starts_segment(one, j + 1U);
            }
            if (!drawn) {
                continue;
            }
...
/*
 * The window one series' readings are listed over: inkcell_trend_frame()'s cut, without the
 * domain - a list has no vertical to contract.
 *
 * A single reading is a window of no width, which inkcell_series_window() declines and this
 ...
 */
static bool trend_reading_window(const struct inkcell_series *series, uint8_t span,
                                 uint32_t *out_from, uint32_t *out_to) {
    if (series == NULL || series->count == 0U) {
        return false;
    }
    const struct inkcell_series *one[1] = {series};
    uint32_t from = 0U;
    uint32_t to = 0U;
    if (!inkcell_series_window(one, 1U, &from, &to)) {
        const struct inkcell_sample *only = inkcell_series_newest(series);
        if (only == NULL) {
            return false;
        }
        from = only->time;
...
bool inkcell_trend_reading_at(const struct inkcell_series *series, uint8_t span, uint32_t index,
    ...
    for (uint32_t i = series->count; i > 0U; --i) {
        const struct inkcell_sample *sample = inkcell_series_at(series, i - 1U);
        if (sample == NULL || sample->time < from || sample->time > to) {
            continue;
        }
        if (seen++ != index) {
...
bool inkcell_trend_frame(const struct inkcell_series *const *series, uint32_t count,
    ...
    uint32_t from = 0U;
    uint32_t to = 0U;
    if (!inkcell_series_window(series, count, &from, &to)) {
        return false;
    }
    ...
    int32_t high = 0;
    struct inkcell_scale scale = domain;
    if (series_high(series, count, from, to, &high)) {
        scale = inkcell_trend_domain(domain, high);
    }
