#ifndef INKCELL_BACKENDS_FB_CAPTURE_H
#define INKCELL_BACKENDS_FB_CAPTURE_H

/*
 * The device UI drawn into memory instead of onto /dev/fb0.
 *
 * inkcell_fb_render() only ever needs a buffer and a geometry, so the same renderer that
 * paints the Brick's panel can paint a malloc'd page on a build host that has no framebuffer -
 * which is every CI runner, every container, and every cloud session. That makes a UI change
 * reviewable as a picture before anyone has a Brick in hand.
 *
 * The page is byte-for-byte what /dev/fb0 holds on the device (32 bpp, B,G,R,X per pixel), so
 * a frame from here and a frame from `deploy-device.sh shot` go through the same encoder.
 */

#include "inkcell/ui/theme.h"

#include <stdbool.h>
#include <stddef.h>
#include <stdint.h>

/* The Brick's panel. Any geometry renders, but this is the one that matches the hardware. */
#define INKCELL_CAPTURE_WIDTH 1024U
#define INKCELL_CAPTURE_HEIGHT 768U

struct inkcell_capture;
struct inkcell_draw_state;

/*
 * Allocates an off-screen page. The theme is the one <PREFIX>_THEME names, as on the device;
 * `scale` is the glyph multiplier the fb backend takes from <PREFIX>_FB_SCALE, clamped to the
 * range that backend accepts - pass 0 for the theme's own. Returns 0, or a negative errno.
 */
int inkcell_capture_open(struct inkcell_capture **out, uint32_t width, uint32_t height, int scale);
void inkcell_capture_close(struct inkcell_capture *capture);
/* Reference rendering for pixel comparisons and CPU measurements. */
void inkcell_capture_set_reference(struct inkcell_capture *capture, bool reference);

/*
 * The drawing state behind the capture, so a harness can install the same app it installs on the
 * device - inkcell_fb_set_app() - and so name the pictures a scene is of.
 *
 * Named rather than discovered, which is the difference between this and the device backend: a
 * frame that quietly picked up whatever the developer happened to have installed would render
 * differently on two machines. A scene says what it is a picture of, or gets nothing.
 *
 * An app that fills one piece per frame does so here exactly as it does on the device, so a
 * scene holds for a moment after opening the way it holds for an animation -
 * inkcell_capture_animating() reports a view still filling.
 */
struct inkcell_draw_state *inkcell_capture_state(struct inkcell_capture *capture);

/* Same clamping as the scale passed to open(). */
void inkcell_capture_set_scale(struct inkcell_capture *capture, int scale);

/*
 * Draws subsequent frames with `theme` - NULL meaning the default - and takes its scale.
 *
 * This is what makes a theme reviewable: the same scene script rendered four times is four GIFs
 * of the same interactions in four looks, from a container with no device attached.
 */
void inkcell_capture_set_theme(struct inkcell_capture *capture, const struct inkcell_theme *theme);
const struct inkcell_theme *inkcell_capture_theme(const struct inkcell_capture *capture);

/*
 * The clock subsequent frames are drawn against.
 *
 * On the device this is the monotonic clock. Here it is whatever a scene script says, because
 * an animation rendered against real time would depend on how fast the host got round to the
 * next frame - and a capture whose contents depend on the machine that took it is not a
 * reviewable picture. Naming the time makes a transition reproducible frame for frame.
 *
 * Only ever moves forwards; an earlier reading than the current one is ignored.
 */
void inkcell_capture_advance(struct inkcell_capture *capture, uint32_t ms);
uint64_t inkcell_capture_now(const struct inkcell_capture *capture);

/* Whether the last frame left something mid-transition, and so whether stepping the clock
   would show something new. */
bool inkcell_capture_animating(const struct inkcell_capture *capture);

/* Draws one whole frame over whatever the page held before. */
/* The rows the last frame's paged list had, which a caller driving the app hands back to it
   before a press - the controller's job on the device. */
uint32_t inkcell_capture_page_rows(const struct inkcell_capture *capture);

void inkcell_capture_render(struct inkcell_capture *capture, const void *snapshot);

/* The page as rendered: `stride` bytes per row, 4 bytes per pixel, B,G,R,X in memory. */
const uint8_t *inkcell_capture_pixels(const struct inkcell_capture *capture, uint32_t *width,
                                      uint32_t *height, size_t *stride);

/*
 * Writes the page as a binary PPM (P6).
 *
 * PPM rather than PNG because a PNG needs deflate, and the one thing this tree must not grow
 * for a developer convenience is a zlib dependency in the aarch64 static link. Compression is
 * scripts/frames.py's job, on the host, out of the Python standard library.
 *
 * Returns 0, or a negative errno.
 */
int inkcell_capture_write_ppm(const struct inkcell_capture *capture, const char *path);

#endif /* INKCELL_BACKENDS_FB_CAPTURE_H */
