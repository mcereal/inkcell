 ...
 *   inkcell_fb_widgets_*  buttons, chips, list rows, field rows, rules       "what things look like"
 *   inkcell_fb_screens_*  one renderer per screen, one file each             "what is on this screen"
 *
 ...
 *   inkcell_fb_widgets_chrome.h    the app bar, the navigation and action bars, the banner, the rule
 *   inkcell_fb_widgets_list.h      the list window, its cards and rail, the subheader and the note
 *   inkcell_fb_widgets_item.h      one list row and its slots, and the conversation cell
 ...
 *   inkcell_fb_widgets_control.h   the switch, the checkbox and radio, the segmented button, the field
 *   inkcell_fb_widgets_meter.h     a quantity as a length, and a reading over time
 ...
 * than this backend's. A screen names one, the theme answers, and inkcell_fb_tone_color() on the state
 ...
 *
 * `struct inkcell_fb_rect` is not here: a box in pixels is the drawing layer's vocabulary rather than
 * any one component's, so it sits in inkcell_fb_internal.h with the rest of the geometry.
 *
