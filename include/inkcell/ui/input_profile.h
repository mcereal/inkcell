#ifndef INKCELL_INPUT_PROFILE_H
#define INKCELL_INPUT_PROFILE_H
 ...
 * One device's pad.
 *
 * `caps` is indexed by enum inkcell_button and is stated in full, including the entries no
 * profile is ever likely to change - a profile that filled in only its differences would be a
 * device described by what it is not. INKCELL_BUTTON_QUIT is the one entry left NULL on
 * purpose: the key that leaves the pak is whatever <PREFIX>_QUIT_KEYS says, so the module
 * that parsed that variable is the one that can name it. See inkcell_button_cap().
 *
 ...
 */
struct inkcell_input_profile {
    const char *name; /* what <PREFIX>_INPUT_PROFILE matches, lower case */
    const struct inkcell_input_binding *bindings;
    size_t binding_count;
    const char *const *caps; /* INKCELL_BUTTON_COUNT entries */
};
...
/*
 * The profile <PREFIX>_INPUT_PROFILE names, or the default. Never NULL.
 *
...
/*
 * The key this profile binds `code` to, or INKCELL_KEY_NONE when it binds nothing to it -
 ...
 * anything outside the enum). Never NULL, for the same reason inkcell_button_cap() is not: a
