#ifndef INKCELL_LATENCY_H
#define INKCELL_LATENCY_H
 ...
 *
 * Off unless it is switched on (<PREFIX>_LATENCY_TRACE, or --trace-latency, which is the one
 ...
 * price is resolution - INKCELL_LATENCY_FINE_US below - and the report says so rather than
...
#define INKCELL_LATENCY_FINE_US 50U
#define INKCELL_LATENCY_FINE_BUCKETS 256U /* 0 .. 12.8 ms */
#define INKCELL_LATENCY_COARSE_US 1000U
#define INKCELL_LATENCY_COARSE_BUCKETS 128U /* 12.8 ms .. 140.8 ms */
#define INKCELL_LATENCY_BUCKETS (INKCELL_LATENCY_FINE_BUCKETS + INKCELL_LATENCY_COARSE_BUCKETS + 1U)
 ...
 */
#define INKCELL_LATENCY_PRESS_TIMEOUT_US 1000000U

struct inkcell_latency_histogram {
    uint32_t buckets[INKCELL_LATENCY_BUCKETS];
    uint32_t count;
    uint32_t min_us;
    uint32_t max_us;
    uint64_t total_us;
...
enum inkcell_latency_metric {
    INKCELL_LATENCY_PRESS = 0,
    INKCELL_LATENCY_FRAME,
    INKCELL_LATENCY_DRAW,
    INKCELL_LATENCY_FLIP,
    INKCELL_LATENCY_READ,
 ...
 * does not use - publishes no snapshot and draws no frame. inkcell_latency_press_handled()
