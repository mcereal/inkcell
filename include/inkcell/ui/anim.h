#ifndef INKCELL_ANIM_H
#define INKCELL_ANIM_H
 ...
 * value from 0 to INKCELL_ANIM_ONE and decides for itself what to do with it, the same way a
 ...
 * The keyed table is `struct inkcell_anim_table` below - the immediate-mode trick, where the
 ...
 */
enum inkcell_ease {
    INKCELL_EASE_LINEAR = 0, /* constant speed: what a progress bar wants */
    INKCELL_EASE_OUT,        /* fast, then settling: what a control the user just pressed wants */
    INKCELL_EASE_IN_OUT,     /* eases at both ends: what a thing moving of its own accord wants */
    INKCELL_EASE_COUNT
};

/*
 * Applies a curve to a linear progress. Both in and out are 0..INKCELL_ANIM_ONE, and both ends
...
struct inkcell_anim {
    ...
    uint8_t ease;         /* enum inkcell_ease */
};
...
/* Eased position at `now_ms`, 0..INKCELL_ANIM_ONE. Clamps outside the window, so a clock that
 ...
 */
#define INKCELL_ANIM_LOOP_STALE_MS 250U

struct inkcell_anim_slot {
    uint32_t id; /* 0 means free */
    uint64_t touched_ms;
    struct inkcell_anim anim;
    /* A sawtooth rather than a transition: see inkcell_anim_loop(). The two are mutually
...
struct inkcell_anim_table {
    struct inkcell_anim_slot slots[INKCELL_ANIM_SLOTS];
};
...
/*
 * A value that runs 0 -> INKCELL_ANIM_ONE over `period_ms` and then starts again.
 *
 ...
 * to be.
 *
 * Unlike a transition this is never "finished", so inkcell_anim_table_active() reports it as
 * running for INKCELL_ANIM_LOOP_STALE_MS after the last frame that drew it - see there.
 *
