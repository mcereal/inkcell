#ifndef INKCELL_BACKENDS_FB_WIDGETS_CONTROL_H
#define INKCELL_BACKENDS_FB_WIDGETS_CONTROL_H
 ...
 * (include/inkcell/ui/anim.h). A screen therefore passes identity, not position, and gets the
 * transition for free.
 *
...
enum inkcell_fb_selection_shape {
    /* A square: any number of these may be on, and this one's state says nothing about its
       neighbours. */
    INKCELL_FB_SELECTION_CHECKBOX = 0,
...
#define INKCELL_FB_SEGMENTED_MAX 4U

struct inkcell_fb_segmented {
    const char *labels[INKCELL_FB_SEGMENTED_MAX];
    size_t count;
