#ifndef INKCELL_BACKENDS_FB_WIDGETS_METER_H
#define INKCELL_BACKENDS_FB_WIDGETS_METER_H
...
enum inkcell_fb_meter_kind {
     ...
     */
    INKCELL_FB_METER_INDETERMINATE,
};

/*
 * A meter's band is `struct inkcell_band` (include/inkcell/ui/theme.h), and it lives there rather
 ...
 * framebuffer exists. inkcell_band_tone() is what reads it.
 *
...
struct inkcell_fb_meter {
     ...
     */
    uint32_t id;
    enum inkcell_fb_meter_kind kind;
...
struct inkcell_fb_slider {
    ...
    /* The active track and the handle, from one tone. ACCENT, GOOD or BAD - the meter's three,
       validated against INKCELL_COLOR_METER_TRACK - and anything else falls back to the
       accent. */
    enum inkcell_tone tone;
    /*
...
#define INKCELL_FB_SIGNAL_CELLS 2U

/*
 * Draws `level` of INKCELL_SIGNAL_STEPS rungs inside `box`, rising left to right.
 *
 ...
 * that was looking.
 *
 * So it draws a memory, and the memory is `struct inkcell_series` (include/inkcell/ui/layout.h) -
 ...
 *
 *   - **The vertical is the reading's own domain**, the same `struct inkcell_scale` the bar
 ...
 *     drawing a slope across it claims readings nobody took - see `gap` on struct inkcell_point.
 ...
 *
 * The slices are laid out by inkcell_proportion_split() (include/inkcell/ui/layout.h), which is
...
struct inkcell_fb_proportion {
    struct inkcell_fb_rect rect; /* inkcell_fb_proportion_thickness() is the height one wants */
    ...
    uint32_t values[INKCELL_PROPORTION_PARTS];
    uint32_t count;
    /*
 ...
 *     `struct inkcell_scale` and never the range these samples happened to span - the sparkline's
...
/* Lines one chart may carry - the palette's count, because a line takes a series colour and two
   ...
   inkcell_fb_draw_proportion() holds the two halves of that seam equal. */
#define INKCELL_FB_CHART_LINES INKCELL_SERIES_COLORS

struct inkcell_fb_chart_line {
    /*
     * The samples, already normalised, exactly as the sparkline takes them - but projected by
     * inkcell_series_project_over() rather than by inkcell_series_project(), and that difference
     ...
     * drawn as though it were still arriving. See inkcell_series_window().
     *
     * Borrowed for the call. Nothing here keeps it.
     */
    const struct inkcell_polyline *points;
 ...
 * with nothing behind it. See struct inkcell_trend_reading.
 */
struct inkcell_fb_chart_reading {
    const char *when;
    const char *value;
};
...
struct inkcell_fb_chart {
    ...
    struct inkcell_fb_rect rect;
    struct inkcell_fb_chart_line lines[INKCELL_FB_CHART_LINES];
    uint32_t count;
    /*
    ...
    /*
     * What to say in the middle of the plot when no line could be drawn in it, or INKCELL_STR_NONE
     * to leave it empty.
     *
     ...
     */
    const struct inkcell_fb_chart_reading *readings;
