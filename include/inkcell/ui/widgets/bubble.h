#ifndef INKCELL_BACKENDS_FB_WIDGETS_BUBBLE_H
#define INKCELL_BACKENDS_FB_WIDGETS_BUBBLE_H
 ...
 * mark that says the message failed is not. It is the same correction inkcell_fb_list_item's slots
...
struct inkcell_fb_bubble_meta {
     ...
     * mark a translator could delete. INKCELL_ICON_NONE for a message that is not one.
     */
    enum inkcell_icon lock;
    ...
    /* What became of one of ours: the clock, the double tick or the alert circle that
       src/ui/tables/delivery.c answers with. INKCELL_ICON_NONE on anything inbound, and on a
       broadcast that went out without want_ack - there is nothing to be waiting for. */
    enum inkcell_icon state;
};
...
struct inkcell_fb_bubble {
     ...
     */
    enum inkcell_tone separator_tone;
