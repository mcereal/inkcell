#ifndef INKCELL_BACKENDS_FB_WIDGETS_ITEM_H
#define INKCELL_BACKENDS_FB_WIDGETS_ITEM_H
...
enum inkcell_fb_trailing_kind {
    ...
    /*
     * A small set of alternatives with all of them on screen - see struct inkcell_fb_segmented.
     *
...
struct inkcell_fb_trailing {
    enum inkcell_fb_trailing_kind kind;
    ...
    /* SIGNAL: rungs lit, 0..INKCELL_SIGNAL_STEPS. inkcell_signal_level() is what answers it, so
...
struct inkcell_fb_list_item {
     ...
     * A tone cannot say "whatever the row is": INKCELL_TONE_NORMAL is the zero, so a field
     ...
     * "whatever ordinary is" without INKCELL_TONE_NORMAL's zero flattening the rows that meant
    ...
    enum inkcell_icon supporting_icon;
    enum inkcell_tone supporting_tone;
    ...
    struct inkcell_fb_trailing supporting_trailing;
 ...
 * inkcell_fb_draw_conversation(), because they have to stay in proportion as a theme changes it.
 */
struct inkcell_fb_conversation {
    ...
    /*
     * What the preview line says when there is no preview to show, as string ids rather than
     * text: the words belong to the application, and only it knows what an empty thread or an
     * armed row means in the language it speaks.
     *
     * `armed_label` replaces the preview entirely while `armed` is set - the row says what the
     * next press does, in place of the thing that press would take away. `empty_label` is for a
