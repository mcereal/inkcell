#ifndef INKCELL_BACKENDS_FB_WIDGETS_BUTTON_H
#define INKCELL_BACKENDS_FB_WIDGETS_BUTTON_H
 ...
 */
enum inkcell_fb_button_variant {
    INKCELL_FB_BUTTON_TEXT = 0, /* nothing until the cursor arrives: a keyboard's character keys */
    INKCELL_FB_BUTTON_FILLED,   /* always visibly a control: the keyboard's action row */
...
struct inkcell_fb_button {
    ...
    enum inkcell_shape shape; /* how round; INKCELL_SHAPE_FULL is a pill */
    /* Label tone for a button showing no fill at all, which is INKCELL_FB_BUTTON_TEXT at rest and
     ...
     *
     * INKCELL_COLOR_BG is the zero value and the usual answer - a keyboard key sits on the body
     * ground. A tab does not: the strip fills its own bar in INKCELL_COLOR_SURFACE_LOW first,
     ...
     * INKCELL_SCALE_MAX and the key is several times a cell.
     *
...
/* What one chip takes, its trailing gap included - so a strip can ask whether it fits before it
   draws anything. The same arithmetic inkcell_fb_draw_chip() advances by, because a strip that measured
 ...
 *
 * inkcell_fb_draw_chip()'s own comment predicted this: a tab strip, a filter row and a segmented
 ...
 * to re-derive the measuring loop, which is the exact duplication inkcell_fb_chip_width() was added to
...
struct inkcell_fb_chip {
     ...
     * than to inkcell_fb_draw_chip(). The strip is what knows whether the labels have been elided, and
...
enum inkcell_fb_chip_labels {
    INKCELL_FB_CHIP_LABELS_ALL = 0,
    INKCELL_FB_CHIP_LABELS_SELECTED, /* only the active chip keeps its words */
    INKCELL_FB_CHIP_LABELS_NONE,     /* icons alone */
};
...
/* The most labels that fit in `room`. INKCELL_FB_CHIP_LABELS_NONE when even the icons overrun, which
 ...
 * (INKCELL_FB_TRAILING_BADGE) and the top app bar's, which is why it is a component of its own rather
 ...
 * against its edge.
 *
 * Same shape and the same measurement - inkcell_fb_badge_width() answers for both - and a different
