#ifndef INKCELL_LAYOUT_H
#define INKCELL_LAYOUT_H
 ...
 *     the edge of the panel. `struct inkcell_line` only ever measures in drawn cells, so the
 ...
 *     find-the-first-visible-row, loop-while-it-fits three-liner. `struct inkcell_list` is
 *     that arithmetic once.
 *   - Wrapping text twice. A chat bubble measures itself and then draws itself, and the two
 *     walks have to agree to the row or bubbles overlap. `struct inkcell_wrap` is the one walk
 *     both passes make, and `inkcell_transcript_window` is the bottom-anchored scroll window
 ...
 *
 * A cell is what `inkcell_text_cell_next` says it is - one character, or one emoji however
 * many codepoints it is spelled with. See include/inkcell/ui/emoji.h.
 */
 ...
 * still logs, and still serialises) as valid text. Build with inkcell_line_reset(), read with
 * inkcell_line_text().
 */
struct inkcell_line {
...
void inkcell_line_reset(struct inkcell_line *line);

/* Append formatted text. */
void inkcell_line_printf(struct inkcell_line *line, const char *fmt, ...)
#if defined(__GNUC__)
    __attribute__((format(printf, 2, 3)))
#endif
    ;
void inkcell_line_vprintf(struct inkcell_line *line, const char *fmt, va_list args)
#if defined(__GNUC__)
 ...
 *
 * The counterpart to inkcell_line_printf() for text the user reads: `id` names the string and
 * inkcell_str_format() supplies it, so a renderer never holds an English sentence. Plain entries
 ...
 */
void inkcell_line_column(struct inkcell_line *line, const char *text, size_t cols);

/* Pad with spaces until the line occupies `cols` cells. A no-op once it is that wide. */
void inkcell_line_pad_to(struct inkcell_line *line, size_t cols);
 ...
 */
void inkcell_line_right(struct inkcell_line *line, size_t cols, const char *right);

/* Clip to `cols` cells. */
void inkcell_line_fit(struct inkcell_line *line, size_t cols);

/* Cells the line occupies once drawn. */
size_t inkcell_line_width(const struct inkcell_line *line);
 ...
 * inkcell_wrap_next(), so they cannot disagree.
 *
 ...
 *
 * struct inkcell_list cannot do this - it assumes every item is one row (or a fixed number of
 ...
 * underneath it. Iterate with inkcell_list_next().
 *
 * ---- steps ----
 *
 ...
 *
 * Every item being one step is the common case and is what inkcell_list_begin() means, so
 ...
 * caller has already walked its items to build them - see inkcell_transcript_window(), which
 ...
 * divides by zero.
 */
struct inkcell_list inkcell_list_begin(uint32_t count, uint32_t cursor, uint32_t visible);
 ...
 */
struct inkcell_list inkcell_list_begin_step(uint32_t count, uint32_t cursor, uint32_t capacity,
                                            uint8_t step);
 ...
 * is what inkcell_list_begin() does and is why scrolling down a list of mixed heights does not
 * feel like a different list.
 */
struct inkcell_list inkcell_list_begin_heights(uint32_t count, uint32_t cursor, uint32_t capacity,
                                               const uint8_t *heights);
 ...
 */
struct inkcell_list inkcell_list_begin_span(uint32_t count, uint32_t cursor, uint32_t span_first,
...
uint8_t inkcell_list_item_height(const struct inkcell_list *list, uint32_t index);

/* Hands back each visible index in turn, false when the window is exhausted. */
bool inkcell_list_next(struct inkcell_list *list, uint32_t *index);

bool inkcell_list_is_cursor(const struct inkcell_list *list, uint32_t index);

/* Where the window starts so that `cursor` is inside it, for a list of one-step items. Answered
   by inkcell_list_begin() rather than derived beside it - one answer for where a list starts. */
 ...
 * are both already on (INKCELL_ANIM_ONE). Clamped to the ends, because a reading off the air
 ...
 */
int32_t inkcell_scale_permille(struct inkcell_scale scale, int32_t value);
 ...
 */
int32_t inkcell_percent_permille(float percent);

/*
 * A temperature the radio reports as a float, as the tenths of a degree the rest of this speaks.
 *
 * The same job inkcell_percent_permille() does one line up and the same reason for existing -
 ...
 */
int32_t inkcell_temperature_decidegrees(float celsius);
 ...
 */
int32_t inkcell_snr_db(float snr);
 ...
 */
#define INKCELL_SNR_EXCELLENT 5
#define INKCELL_SNR_GOOD 0
#define INKCELL_SNR_FAIR (-7)
#define INKCELL_SNR_POOR (-15)
...
#define INKCELL_SNR_FLOOR (-20)
#define INKCELL_SNR_CEILING 10
 ...
 * why inkcell_signal_level() counts its rungs off SNR and not off this. What RSSI says is how
 ...
 */
#define INKCELL_RSSI_FAIR (-100)
#define INKCELL_RSSI_POOR (-115)
#define INKCELL_RSSI_FLOOR (-130)
#define INKCELL_RSSI_CEILING (-30)
...
#define INKCELL_SIGNAL_STEPS 4U

/*
 * How many of the INKCELL_SIGNAL_STEPS rungs `snr` lights: 0 at the floor, 4 for a link
 ...
 */
uint8_t inkcell_signal_level(float snr);
 ...
 */
#define INKCELL_AIRTIME_BUSY_WARN 250
#define INKCELL_AIRTIME_BUSY_BAD 500
#define INKCELL_AIRTIME_TX_WARN 50
#define INKCELL_AIRTIME_TX_BAD 100
 ...
 */
#define INKCELL_BATTERY_LOW 30
#define INKCELL_BATTERY_CRITICAL 15
 ...
 * actually carry, and the same reason inkcell_percent_permille() exists one reading over: a
 * whole degree would put a staircase under a reading that was holding still.
 */
#define INKCELL_TEMPERATURE_WARM 500
#define INKCELL_TEMPERATURE_HOT 600
 ...
 */
#define INKCELL_TEMPERATURE_FLOOR (-400)
#define INKCELL_TEMPERATURE_CEILING 800
 ...
 *
 * Permille because inkcell_percent_permille() is where every percentage off the air becomes an
 * integer, and a second unit for this one would be a second rounding.
 */
#define INKCELL_HUMIDITY_DAMP 800
#define INKCELL_HUMIDITY_WET 900
 ...
 */
#define INKCELL_IAQ_FLOOR 0
#define INKCELL_IAQ_CEILING 500
#define INKCELL_IAQ_POLLUTED 150
#define INKCELL_IAQ_HEAVY 250

#define INKCELL_CO2_FLOOR 400
#define INKCELL_CO2_CEILING 3000
#define INKCELL_CO2_STUFFY 1000
#define INKCELL_CO2_BAD 2000

#define INKCELL_PM25_FLOOR 0
#define INKCELL_PM25_CEILING 75
#define INKCELL_PM25_ELEVATED 15
#define INKCELL_PM25_UNHEALTHY 35
...
/* Parts one composition may have. It is INKCELL_SERIES_COLORS (include/inkcell/ui/theme.h) counted
   ...
   equal where they meet, by the static assertion in src/fb/widgets_meter.c. Four is the number
 ...
 * draw at all: no parts, no extent, more parts than INKCELL_PROPORTION_PARTS, or a whole that
 ...
 */
uint32_t inkcell_proportion_split(const uint32_t *values, uint32_t count, int32_t extent,
                                  int32_t *out);
 ...
 *   - **The y axis is the reading's own domain**, the same `struct inkcell_scale` the bar beside
 ...
 *     landed on (inkcell_trend_domain()); the floor stays where the reading's domain put it, and
 ...
 * taken before it into the far past. inkcell_series_push() drops the whole series if the clock
...
#define INKCELL_SERIES_MAX 24U

struct inkcell_sample {
    ...
    /* This reading does not continue the one before it, for a reason the clock cannot show -
       see inkcell_series_break(). The elapsed-time test is the other way a point becomes the
       start of a segment, and the two are independent. */
    bool gap;
};

struct inkcell_series {
    struct inkcell_sample items[INKCELL_SERIES_MAX];
    ...
    uint32_t gap_ms;
    /* inkcell_series_break() has been called and no reading has arrived since. */
    bool pending_break;
};

/* Empties it and states how long a silence counts as a break. */
void inkcell_series_reset(struct inkcell_series *series, uint32_t gap_ms);
 ...
 * readings the clock could not separate are still two readings, and inkcell_series_project()
 * says what it does with them.
 */
void inkcell_series_push(struct inkcell_series *series, uint32_t time, int32_t value);
 ...
 */
void inkcell_series_break(struct inkcell_series *series);

/* The `index`th oldest, or NULL past the end. */
const struct inkcell_sample *inkcell_series_at(const struct inkcell_series *series, uint32_t index);

/* The newest, or NULL for an empty series - the reading a caller would draw a figure from. */
const struct inkcell_sample *inkcell_series_newest(const struct inkcell_series *series);
 ...
 * `gap_ms`, and every sample the source itself broke before (inkcell_series_break()).
 */
struct inkcell_point {
    int16_t x;
    int16_t y;
    bool gap;
};

struct inkcell_polyline {
    struct inkcell_point items[INKCELL_SERIES_MAX];
    uint32_t count;
};
 ...
 */
void inkcell_series_project(const struct inkcell_series *series, struct inkcell_scale scale,
                            struct inkcell_polyline *out);
 ...
 * same break test inkcell_series_project_over() applies, because the alternative is a predicate
 * that can say a line exists and a projection that declines to draw one.
 */
bool inkcell_series_has_segment(const struct inkcell_series *series);

/*
 * Whether the `index`th reading begins a segment rather than continuing the one before it.
 *
 * The same test the projection lifts the pen on and inkcell_series_has_segment() counts, asked of
 ...
 * the picture. inkcell_trend_domain()'s ceiling is the case that found it - an isolated reading
 ...
 */
bool inkcell_series_starts_segment(const struct inkcell_series *series, uint32_t index);
 ...
 */
bool inkcell_series_window(const struct inkcell_series *const *series, uint32_t count,
                           uint32_t *out_from, uint32_t *out_to);

/*
 * The same projection, over a window stated by the caller rather than over the series' own.
 *
 * inkcell_series_project() is this with the series' own two ends, which is the right window for
 * a line drawn by itself. What a chart wants instead is one window across everything on it - see
 * inkcell_series_window() - so that a series which stopped reporting ends where it stopped
 ...
 * `to` at or before `from` falls back to even spacing, which is inkcell_series_project()'s own
 * answer for a span of zero and is here for the same reason - the order of the samples is then
 * all that is known about them.
 */
void inkcell_series_project_over(const struct inkcell_series *series, struct inkcell_scale scale,
 ...
 * series themselves (inkcell_series_window()) contains every sample by construction, so nothing
