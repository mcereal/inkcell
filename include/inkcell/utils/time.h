#ifndef INKCELL_TIME_H
#define INKCELL_TIME_H
 ...
 */
uint64_t inkcell_time_monotonic_ms(void);
 ...
 */
uint32_t inkcell_time_wall_s(void);
 ...
 */
#define INKCELL_TIME_CLOCK_MIN_EPOCH 1600000000U

/*
 * The wall clock when it is credibly one, and 0 when it is not.
 *
 * inkcell_time_wall_s() answers whatever the machine says, which is what a "3m ago" wants: an age
 ...
 */
uint32_t inkcell_time_wall_credible_s(void);

/*
 * Freeze inkcell_time_wall_s() at `epoch`, or pass 0 to follow the real clock again. Devtools and
