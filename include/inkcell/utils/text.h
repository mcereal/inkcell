#ifndef INKCELL_TEXT_H
#define INKCELL_TEXT_H
...
#define INKCELL_TEXT_INVALID_CODEPOINT 0xFFFDU
 ...
 */
size_t inkcell_text_utf8_sequence_len(const uint8_t *bytes, size_t available);
 ...
 * A malformed byte consumes one byte and yields INKCELL_TEXT_INVALID_CODEPOINT, so a caller that
 * loops on the return value always makes progress.
 */
size_t inkcell_text_utf8_next(const char *text, uint32_t *codepoint);
...
size_t inkcell_text_utf8_length(const char *text);
...
size_t inkcell_text_utf8_offset(const char *text, size_t index);

/* Truncate `text` in place to at most `max_chars` characters. */
void inkcell_text_utf8_truncate(char *text, size_t max_chars);
 ...
 */
void inkcell_text_sanitise(const uint8_t *payload, size_t len, char *out, size_t out_len);

/* inkcell_text_sanitise for a NUL-terminated source. `in` may be NULL, which yields "". */
void inkcell_text_sanitise_str(const char *in, char *out, size_t out_len);
 ...
 *
 * Truncation cuts at a byte, not a character. Callers rendering UTF-8 want inkcell_text_sanitise()
 * or a inkcell_text_utf8_truncate() afterwards.
 */
bool inkcell_str_copy(char *out, size_t out_len, const char *in);

#ifdef __cplusplus
}
#endif

#endif /* INKCELL_TEXT_H */
