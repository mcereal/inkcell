 ...
 *
 * inkcell writes the log; what else wants a copy is the application's business. A crash
 * reporter keeping the last few lines is the case this exists for, and it is a sink rather than
 * something inkcell calls by name because a UI library has no business knowing whether the app
...
enum inkcell_log_level {
    INKCELL_LOG_LEVEL_TRACE = 0,
    INKCELL_LOG_LEVEL_DEBUG,
    INKCELL_LOG_LEVEL_INFO,
    INKCELL_LOG_LEVEL_WARN,
    INKCELL_LOG_LEVEL_ERROR,
    INKCELL_LOG_LEVEL_NONE
};
...
static inline void inkcell_log_debug(const char *component, const char *fmt, ...)
    __attribute__((format(printf, 2, 3)));
static inline void inkcell_log_info(const char *component, const char *fmt, ...)
    __attribute__((format(printf, 2, 3)));
static inline void inkcell_log_warn(const char *component, const char *fmt, ...)
    __attribute__((format(printf, 2, 3)));
static inline void inkcell_log_error(const char *component, const char *fmt, ...)
    __attribute__((format(printf, 2, 3)));

static inline void inkcell_log_trace(const char *component, const char *fmt, ...) {
    va_list args;
    va_start(args, fmt);
    inkcell_log_message_v(INKCELL_LOG_LEVEL_TRACE, component, fmt, args);
    va_end(args);
}
...
static inline void inkcell_log_debug(const char *component, const char *fmt, ...) {
    va_list args;
    va_start(args, fmt);
    inkcell_log_message_v(INKCELL_LOG_LEVEL_DEBUG, component, fmt, args);
    va_end(args);
}
...
static inline void inkcell_log_info(const char *component, const char *fmt, ...) {
    va_list args;
    va_start(args, fmt);
    inkcell_log_message_v(INKCELL_LOG_LEVEL_INFO, component, fmt, args);
    va_end(args);
}
...
static inline void inkcell_log_warn(const char *component, const char *fmt, ...) {
    va_list args;
    va_start(args, fmt);
    inkcell_log_message_v(INKCELL_LOG_LEVEL_WARN, component, fmt, args);
    va_end(args);
}
...
static inline void inkcell_log_error(const char *component, const char *fmt, ...) {
    va_list args;
    va_start(args, fmt);
    inkcell_log_message_v(INKCELL_LOG_LEVEL_ERROR, component, fmt, args);
    va_end(args);
}
...
#define INKCELL_LOG_FILE_MAX_BYTES (512U * 1024U)
#define INKCELL_LOG_FILE_KEEP_BYTES (128U * 1024U)

/* Enough for the logs directory plus an application name and the extension: a path here is a
   value, not an allocation. */
#define INKCELL_LOG_FILE_PATH_MAX 256U
 ...
 */
bool inkcell_log_file_default_path(char *out, size_t out_len);

/*
 * Cut `path` back to its newest INKCELL_LOG_FILE_KEEP_BYTES when it has outgrown the cap.
 *
