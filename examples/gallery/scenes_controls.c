 ...
 * that is on and not selected has to read as on against the body ground, and a control that is
 * selected and off has to read as off against the cursor fill.
 *
 ...
 * and selected drew itself checked: it had collided with a switch that was on, read that
...
void gallery_scene_controls(struct inkcell_backend_fb_state *state) {
    ...
    for (int on = 0; on < 2; ++on) {
        for (int selected = 0; selected < 2; ++selected) {
            const struct inkcell_fb_switch sw = {
                ...
                .id = (uint32_t)(GALLERY_ANIM_SWITCH + on * 2 + selected),
    ...
    for (int selected = 0; selected < 2; ++selected) {
        const struct inkcell_fb_rect rect = {
            .x = box.text_x + selected * (seg_w + gap), .y = y, .w = seg_w, .h = seg_h};
        inkcell_fb_draw_segmented(state, &rect, &spans, selected != 0, INKCELL_COLOR_BG, scale);
    }
    y += seg_h + gap;
