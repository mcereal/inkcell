...
INKCELL_TEST_CASE(list_glide_displaces_the_content_and_settles_it, unit) {
    ...
    const int settled = glide_row_y(&h, visible);
    ...
    const int displaced = glide_row_y(&g, visible);
    ...
    (void)glide_frame(&g, visible, GLIDE_ID);
    const int midway = glide_row_y(&g, visible);
    ...
    (void)glide_frame(&g, visible, GLIDE_ID);
    INKCELL_TEST_FAIL_IF_CLEANUP(glide_row_y(&g, visible) != settled,
                                 (glide_close(&h), glide_close(&g)),
