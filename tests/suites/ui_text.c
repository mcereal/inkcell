...
INKCELL_TEST_CASE(font5x7_coverage, unit) {
    ...
    INKCELL_TEST_FAIL_IF(!mark_in_cell, "e-acute should differ from e");

    /* Capitals occupy all seven rows, so their mark goes into the gap above instead. */
    INKCELL_TEST_FAIL_IF(!inkcell_font5x7_glyph(0x00C9U, &accented), "E-acute should have a glyph");
    ...
    for (size_t i = 0; i < sizeof cases / sizeof cases[0]; ++i) {
        ...
        INKCELL_TEST_FAIL_IF(is_box == cases[i].covered, cases[i].label);
    }
 ...
 */
INKCELL_TEST_CASE(ui_text_cells, unit) {
    struct {
        const char *label;
        const char *text;
        size_t cells;
    ...
    for (size_t i = 0; i < sizeof cases / sizeof cases[0]; ++i) {
        INKCELL_TEST_FAIL_IF(inkcell_text_cells(cases[i].text) != cases[i].cells, cases[i].label);
        ...
        INKCELL_TEST_FAIL_IF(cases[i].text[end] != '\0', cases[i].label);
    }
...
INKCELL_TEST_CASE(ui_text_cell_kinds, unit) {
    struct {
        const char *label;
        const char *text;
        bool is_emoji;
 ...
 */
INKCELL_TEST_CASE(emoji_ascii_fast_path_precondition, unit) {
...
INKCELL_TEST_CASE(emoji_table_integrity, unit) {
    const struct inkcell_emoji_table *table = &inkcell_emoji_table;
