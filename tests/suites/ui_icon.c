 ...
 *
 * The enum comes from include/inkcell/ui/icons.def and the sprites are generated from the same
 ...
 */
INKCELL_TEST_CASE(icon_table_covers_every_id, unit) {
    ...
    for (int i = 0; i < (int)INKCELL_ICON_COUNT; ++i) {
        ...
        INKCELL_TEST_FAIL_IF(end < start, "run offsets should not go backwards");
...
INKCELL_TEST_CASE(icon_sprites_have_ink, unit) {
 ...
 *     towards the other.
 */
INKCELL_TEST_CASE(icon_sprites_are_not_cropped, unit) {
    uint8_t alpha[INKCELL_ICON_SIZE * INKCELL_ICON_SIZE];
    char message[160];
...
INKCELL_TEST_CASE(icon_none_is_blank, unit) {
    uint8_t alpha[INKCELL_ICON_SIZE * INKCELL_ICON_SIZE];
    memset(alpha, 0xFF, sizeof alpha);
    inkcell_icon_alpha(INKCELL_ICON_NONE, alpha);
    for (size_t p = 0; p < sizeof alpha; ++p) {
        INKCELL_TEST_FAIL_IF(alpha[p] != 0U, "INKCELL_ICON_NONE should decode to nothing");
    }
...
/* Names are what the generator matched in the font and what a failure message can print. Two
   ...
   parted company. */
INKCELL_TEST_CASE(icon_names_are_present, unit) {
    for (int i = 1; i < (int)INKCELL_ICON_COUNT; ++i) {
