...
INKCELL_TEST_CASE(anim_easings_pin_both_ends, unit) {
    ...
    INKCELL_TEST_FAIL_IF(out <= linear,
                         "ease-out should be ahead of linear a quarter of the way in");
...
INKCELL_TEST_CASE(anim_value_walks_from_start_to_target, unit) {
    ...
    INKCELL_TEST_FAIL_IF(inkcell_anim_value(&anim, 1000U) != 0,
                         "a set value should sit where it is");
...
INKCELL_TEST_CASE(anim_table_adopts_then_animates, unit) {
    ...
    INKCELL_TEST_FAIL_IF(first != INKCELL_ANIM_ONE,
                         "a control should be drawn at its value on sight");
...
INKCELL_TEST_CASE(anim_loop_runs_a_sawtooth_off_the_clock, unit) {
    ...
    INKCELL_TEST_FAIL_IF(inkcell_anim_loop(&table, 1U, 987654U + 250U, 1000U) !=
                             INKCELL_ANIM_ONE / 4,
    ...
    INKCELL_TEST_FAIL_IF(inkcell_anim_loop(&table, 1U, 987654U + 7250U, 1000U) !=
                             INKCELL_ANIM_ONE / 4,
...
INKCELL_TEST_CASE(anim_loop_and_track_do_not_bleed_into_each_other, unit) {
    ...
    INKCELL_TEST_FAIL_IF(
        inkcell_anim_track(&table, 7U, 1600U, INKCELL_ANIM_ONE / 2, 200U, INKCELL_EASE_OUT) !=
            INKCELL_ANIM_ONE / 2,
...
INKCELL_TEST_CASE(anim_null_arguments_are_safe, unit) {
    ...
    INKCELL_TEST_FAIL_IF(inkcell_anim_table_active(NULL, 1000U),
                         "a NULL table should not be active");
