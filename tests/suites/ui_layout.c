...
INKCELL_TEST_CASE(ui_layout_scroll_reports_the_window, unit) {
    ...
    INKCELL_TEST_FAIL_IF(inkcell_list_thumb(&empty, 400, 8).length != 0,
                         "an empty list asked for a thumb");
    INKCELL_TEST_FAIL_IF(inkcell_list_thumb(NULL, 400, 8).length != 0,
                         "a NULL list drew something");
    struct inkcell_list any = inkcell_list_begin(40U, 0U, 10U);
    INKCELL_TEST_FAIL_IF(inkcell_list_thumb(&any, 0, 8).length != 0, "a track of nothing drew");
